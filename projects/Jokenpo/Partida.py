...
class Partida(Jogadores):
    ...
    def jogar(self):
        ...
        for _ in range(2):
            ...
            rodadas.append(
                f"{self.traduzir_cartas(str(self.jogador_um.cartas[carta_jogador1]))} x {self.traduzir_cartas(str(self.jogador_dois.cartas[carta_jogador2]))}"
            )
    ...
    def simular(self):
        ...
        for round in range(3):
            if self.jogador_um.cartas[round] == self.jogador_dois.cartas[round]:
                print(f"Round({str(round + 1)}) - Empate")
                count_Empate += 1
                if count_Empate == 3:
