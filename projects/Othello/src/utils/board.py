...
class Board:
    ...
    def __init__(self) -> None:
        ...
        self.board = np.array(
            [0] * 8, dtype=np.int8
        )  # initiliasing 1D array with the first row of 8 zeroes
    ...
    def legal_moves(self, r: int, c: int) -> list:
        ...
        for dir in Board.DIRECTIONS:
            ...
            if (
                Board.checkCoordRange(row, col) is False
                or self.board[row, col] != OPPONENT
            ):
                continue
            ...
            col += colDir
            while (
                Board.checkCoordRange(row, col) is True
                and self.board[row, col] == OPPONENT
            ):
                row += rowDir
                col += colDir
            if (
                Board.checkCoordRange(row, col) is True
                and self.board[row, col] == Board.EMPTY
            ):  # possible move
    ...
    def evaluate_board(self) -> int:
        ...
        coin_parity = (
            100
            * (self.black_disc_count - self.white_disc_count)
            / (self.black_disc_count + self.white_disc_count)
        )
        ...
        if black_corners + white_corners == 0:
            ...
            corner_value = (
                100 * (black_corners - white_corners) / (black_corners + white_corners)
            )
