...
class Application:
    ...
    def __init__(self) -> None:
        ...
        self.boardIMG = [
            pygame.image.load("images/Dark-Mode/Othello_Black_Side_Board.png"),
            pygame.image.load("images/Light-Mode/Othello_Black_Side_Board.png"),
        ]

        self.endScreenBlackIMG = [
            pygame.image.load("images/Dark-Mode/End_Screen_Black.png"),
            pygame.image.load("images/Light-Mode/End_Screen_Black.png"),
        ]
        self.endScreenWhiteIMG = [
            pygame.image.load("images/Dark-Mode/End_Screen_White.png"),
            pygame.image.load("images/Light-Mode/End_Screen_White.png"),
        ]
        self.endScreenDrawIMG = [
            pygame.image.load("images/Dark-Mode/End_Screen_Draw.png"),
            pygame.image.load("images/Light-Mode/End_Screen_Draw.png"),
        ]
        self.endPromptIMG = [
            pygame.image.load("images/Dark-Mode/End_Prompt.png"),
            pygame.image.load("images/Light-Mode/End_Prompt.png"),
        ]

        self.choiceIMG = [
            pygame.image.load("images/Dark-Mode/Othello_Game-mode_Choice.png"),
    ...
    @staticmethod
    def fade(
        screen: pygame.Surface, *surfaceNcoords: tuple[pygame.Surface, tuple[int, int]]
    ):
    ...
    def handleMouseClick(self) -> None:
        ...
        for pos in self.possible_moves:
            ...
            y = 100 + 75 * row
            pygame.draw.rect(
                self.screen, self.BG[self.theme], pygame.Rect(x + 4, y + 4, 67, 67)
            )
    ...
    def handleGameEnd(self, event: pygame.event.Event):
        ...
        if event.key not in (pygame.K_r, pygame.K_q):
            return

        # fade out the screen
        dummy_surface = pygame.Surface(
            (Application.SCREEN_WIDTH, Application.SCREEN_HEIGHT)
        )
    ...
    def displayScore(self) -> None:
        ...
        black_disc_count = self.discCountFont.render(
            f"{self.game_board.black_disc_count}", False, text_color
        )
        white_disc_count = self.discCountFont.render(
            f"{self.game_board.white_disc_count}", False, text_color
        )
    ...
    def displayLegalMoves(self) -> None:
        ...
        if not self.possible_moves:
            ...
            self.shown_moves = not (
                self.hasBlackForfeited
                if self.turn == Board.BLACK
                else self.hasWhiteForfeited
            )
            self.turn *= -1
            return
        ...
        surface = pygame.Surface(
            (75, 75), pygame.SRCALPHA
        )  # transparent surface to draw possible moves on
        for pos in self.possible_moves:
            r, c = pos
            x = 100 + 75 * c
            y = 100 + 75 * r
    ...
    def gameOverScreen(self) -> None:
        ...
        if (
            self.game_board.black_disc_count > self.game_board.white_disc_count
        ):  # black won
            Application.fade(
                self.screen, (self.endScreenBlackIMG[self.theme], (725, 250))
            )
            ...
            Application.fade(
                self.screen, (self.endScreenWhiteIMG[self.theme], (725, 250))
            )

        else:  # draw
            Application.fade(
                self.screen, (self.endScreenDrawIMG[self.theme], (725, 250))
            )
